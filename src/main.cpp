...
#if defined(NDEBUG)
# error "Astercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd18ba27f370ce38046faf9327d1254913f9fd5317e1c0ed64d94767fcb1d958b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Astercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Astercoin Signed Message:\n";
...
{
    // Astercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Astercoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Astercoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Astercoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Astercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Astercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1d803f11bc451d9f0653566b6a6358039a611e530cc37d7fd692c7dbde2c6b33");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Putin became the President on March 28";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04eb43ed1e9c8c4e17465df2d89d52bd20aff3d6f647d2e39577905d16fd12e241f9c07dee41d591ed82fb40a13cfbfb83a4846518b9e2bd4ef091df676b23015b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1533716160;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085753082;

        if (fTestNet)
        {
            block.nTime    = 1533716137;
            block.nNonce   = 386208321;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x877ec0292d4cf466a2193dc8d373c8115a45e93fdb6ba4a1d3781d26655f3ca6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Astercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AstercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AstercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AstercoinMiner(CWallet *pwallet)
{
    printf("AstercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("astercoin-miner");
    ...
    try { loop {
        ...
        printf("Running AstercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AstercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AstercoinMiner, pwallet));
}
